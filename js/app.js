...
async function initSupabase() {
  // ✅ 关键修复：在函数内部重新读取配置，确保获取到最新的值
  // 因为模块加载时可能配置还没有设置好
  const currentSupabaseUrl = (window.SUPABASE_URL || localStorage.getItem("xhs_supabase_url") || "").trim();
  const currentSupabaseAnonKey = (window.SUPABASE_ANON_KEY || localStorage.getItem("xhs_supabase_anon_key") || "").trim();
  
  hasSupabase = Boolean(currentSupabaseUrl && currentSupabaseAnonKey);
  if (!hasSupabase) {
    logger.warn('⚠️ Supabase 配置缺失，跳过初始化', {
  ...
  try {
    ...
    // ✅ 设置到 window 对象，方便其他代码访问
    window.supabase = supabase;
    
    // ✅ 设置 Supabase 客户端到 sync.js
    ...
    setDependencies({
      ...
      applyView,
      renderCloudHistory // 递归依赖，需要传递自己
    });
...
let dbReadyPromise = db.open().then(() => {
  logger.log('✅ IndexedDB 数据库连接已就绪');
  return true;
}).catch((err) => {
  logger.error('❌ IndexedDB 数据库连接失败:', err);
...
function readCats() {
  try {
    ...
    logger.log('📦 读取分类数据:', { key: CATS_KEY, raw: raw ? '有数据' : '无数据' });
    if (!raw) {
      logger.log('⚠️ 分类数据为空，返回默认分类');
    ...
    const obj = JSON.parse(raw);
    logger.log('📦 解析后的分类数据:', obj);
    // ✅ 详细显示每个分类的名称和颜色
    if (Array.isArray(obj) && obj.length > 0) {
      logger.log('📋 分类详情:', obj.map(cat => ({
        id: cat.id,
        name: cat.name,
        color: cat.color
      })));
    ...
    if (Array.isArray(obj) && obj.length) {
      ...
      logger.log('✅ 返回分类数据:', fixed);
      // ✅ 再次详细显示返回的分类数据
      logger.log('📋 返回的分类详情:', fixed.map(cat => ({
        id: cat.id,
        name: cat.name,
        color: cat.color
      })));
    ...
    logger.log('⚠️ 分类数据格式不正确，返回默认分类');
    // ✅ 如果数据格式不正确，也保存默认分类
    const defaultCats = DEFAULT_CATS.slice();
    saveCats(defaultCats);
    return defaultCats;
  } catch (err) {
    logger.error('❌ 读取分类数据失败:', err);
    // ✅ 如果读取失败，也保存默认分类
    const defaultCats = DEFAULT_CATS.slice();
    saveCats(defaultCats);
    return defaultCats;
...
function saveCats(cats) {
  logger.log('💾 保存分类数据:', { key: CATS_KEY, cats: cats });
...
async function getCurrentUserId() {
  if (!supabase) {
    logger.warn('⚠️ Supabase 未初始化，返回 null');
    return null;
  }
  ...
  try {
    ...
    if (error) {
      logger.error('获取 Supabase 会话失败:', error);
      return null;
    }
    const uid = session?.user?.id || null;
    if (!uid) {
      logger.error('⚠️ 当前没有登录用户');
    }
    return uid;
  } catch (error) {
    logger.error('获取 Supabase 会话异常:', error);
    return null;
  }
}
...
async function checkPermission(resourceId, resourceType, permissionType = 'view') {
  if (!supabase) {
    logger.warn('⚠️ Supabase 未初始化，跳过权限检查');
    return false;
  }
  ...
  try {
    ...
    if (!authUid) {
      logger.error('❌ 当前没有登录用户，无法执行权限检查');
      return false;
    }

    logger.log('🔍 权限检查:', { resourceId, resourceType, permissionType, authUid });
    ...
    if (resourceType === 'snapshot') {
      ...
      if (snapshotError) {
        logger.error('❌ 查询快照所有者失败:', snapshotError);
        return false;
      }

      if (snapshot?.owner_id === authUid) {
        logger.log('✅ 当前用户是资源所有者');
        return true;
      }
    }
    ...
    if (permError) {
      logger.error('❌ 查询权限记录失败:', permError);
      return false;
    }

    if (!permission) {
      logger.log('❌ 未找到权限记录', { resourceId, resourceType, authUid });
      return false;
    }

    if (permission.expired_at && new Date(permission.expired_at) < new Date()) {
      logger.warn('⚠️ 权限记录已过期，自动标记为 expired');
      await supabase
        .from('permissions')
...
function setActiveFunction(functionName) {
  ...
  if (functionName) {
    ...
    if (btn) {
      ...
      logger.log(`✅ 激活功能按钮: ${functionName}`);
    }
  }
}
...
function applyFilters(rows) {
  ...
  if (!state.sortBy || (state.sortBy !== "owner" && state.sortBy !== "wx_real" && state.sortBy !== "phone" && state.sortBy !== "xhs_name" && state.sortBy !== "row_color" && state.sortBy !== "order")) {
    logger.warn('⚠️ 无效的排序方式，使用默认值 "owner"', state.sortBy);
    state.sortBy = "owner";
  }
  ...
  switch (state.sortBy) {
    case "owner": {
      logger.log("🔍 执行按所属人排序，数据量:", out.length);
      const priority = ["Olina", "嘉", "良", "齐", "齐注销", "宫"];
      const norm = (s) => String(s || "").trim().toLowerCase();
      const priIndex = new Map(priority.map((name, idx) => [norm(name), idx]));
...
async function renderTable() {
  logger.log("🎨 renderTable 被调用");
  logger.log("🔍 当前排序方式:", state.sortBy);
  const tbody = $("#gridBody");
  const all = await getAllRows();
  logger.log(`📊 总共 ${all.length} 行数据`);
  const rows = applyFilters(all);
  logger.log(`📊 过滤后 ${rows.length} 行数据`);
  // ✅ 调试：显示前几条数据的所属人，验证排序是否正确
  if (rows.length > 0 && state.sortBy === "owner") {
    logger.log("🔍 排序验证（前5条数据的所属人）:", rows.slice(0, 5).map(r => r.owner || "(空)"));
  }

  if (!rows.length) {
    tbody.innerHTML = `<tr><td colspan="8" style="text-align:center;color:#888;padding:14px 0;">暂无数据，点击"新增一行"开始录入</td></tr>`;
  } else {
    logger.log("🔧 开始生成 HTML");
    ...
    logger.log(`🔍 表格中共有 ${selects.length} 个分类选择器`);
  }
...
// ✅ cloudHealthCheck, cloudSave, cloudLoad, renderCloudHistory 已移至 sync.js 模块
// 使用 import 导入，不再在此定义

/* =========================
 * 云端同步函数（已移至 sync.js，保留此注释以便查找）
 * ========================= */
...
function renderCatList() {
  const list = $("#catList");
  if (!list) {
    logger.error('❌ 找不到分类列表容器 #catList');
    return;
  }
    ...
    if (metadataKeys.length > 0) {
      logger.warn('⚠️ 检测到 view 数据中包含可能的元数据字段:', metadataKeys);
      logger.warn('⚠️ 这些字段将在数据比较时被排除:', metadataKeys);
    }
    ...
    try {
      ...
      if (cloudCheckError) {
        logger.error('❌ 查询云端快照失败:', cloudCheckError);
        ...
        if (localSnapshotTime && localSnapshotKey === SUPABASE_DEFAULT_KEY) {
          ...
          if (timeDiff > 60000) { // 超过1分钟
            ...
            if (cloudOwnerId !== authUid) {
              ...
              logger.warn('⚠️ 检测到可能的冲突：云端快照不是当前用户保存的');
            }
          }
        }
        ...
        if (hasConflict && conflictInfo) {
          ...
          if (shouldLoad) {
            ...
            logger.warn('⚠️ 用户选择强制保存，可能覆盖云端数据');
          }
        } else {
          logger.log('✅ 未检测到冲突，可以继续保存');
        }
      } else {
        // 云端没有快照（首次保存）
        logger.log('ℹ️ 云端没有快照，允许首次保存');
      }
    } catch (error) {
      logger.error('❌ 检查云端更新时发生异常:', error);
      // 发生异常时，显示警告但允许继续
      const shouldContinue = confirm(
        '⚠️ 检查云端更新时发生错误\n\n' +
    ...
    if (defaultError) {
      logger.warn('⚠️ 查询默认快照失败:', defaultError);
    }
    if (historyError) {
      logger.warn('⚠️ 查询历史快照失败:', historyError);
    }
    
    // ✅ 检查是否有数据改动：比较当前数据和最新快照（在 prompt 之前检查）
    if (latestSnapshot && latestSnapshot.payload) {
      logger.log('✅ 找到最新快照，开始比较数据...', {
      ...
      if (!latestSnapshot) {
        logger.log('ℹ️ 没有找到历史快照，允许保存（首次保存）');
      } else if (!latestSnapshot.payload) {
        logger.log('⚠️ 最新快照没有 payload 数据，允许保存');
      } else {
        logger.log('⚠️ 最新快照检查失败，但继续保存流程', {
    ...
    if (existingError) {
      logger.error('❌ 查询默认快照失败:', existingError);
    ...
    logger.log('✅ 自动生成快照名称:', snapshotName);

    const now = Date.now();

    logger.log('✅ 数据收集完成，准备构建 payload');
    ...
    logger.log('💾 开始保存默认快照...', { ownerId, authUid });
    ...
    if (upsertError) {
      logger.error('❌ 保存默认快照失败:', upsertError);
    ...
    try {
      ...
      if (savedTimeCheck && savedKeyCheck === SUPABASE_DEFAULT_KEY) {
        ...
        // 显示警告但不阻止保存流程
        logger.warn('⚠️ localStorage 更新失败，可能影响下次冲突检测');
      }
    } catch (e) {
      logger.error('❌ 更新版本记录时发生错误:', e);
      // 即使更新失败，也不阻止保存流程
    }
    ...
    if (historyInsertError) {
      logger.warn('⚠️ 保存历史快照失败（不影响主流程）:', historyInsertError);
    } else {
      logger.log('✅ 历史快照保存成功，开始清理旧快照');
    ...
    logger.error('❌ 保存云端时发生未捕获的错误:', error);
    alert(
      "保存失败：发生未知错误\n\n" +
...
function renderCatList() {
  ...
  if (Array.isArray(cloudCats) && cloudCats.length > 0) {
    logger.log('☁️ 云端分类详情:', cloudCats.map(cat => ({
      id: cat.id,
      name: cat.name,
      color: cat.color
    })));
  } else {
    logger.log('ℹ️ 云端没有分类数据，使用本地分类');
  }
  // ✅ 详细显示本地分类数据
  if (localCats.length > 0) {
    logger.log('💾 本地分类详情:', localCats.map(cat => ({
      id: cat.id,
      name: cat.name,
      color: cat.color
    })));
  ...
  // ✅ 详细显示最终使用的分类数据
  logger.log('✅ 最终使用的分类详情:', cats.map(cat => ({
    id: cat.id,
    name: cat.name,
    color: cat.color
  })));
  ...
  if (data && data.updated_at) {
    try {
      ...
      if (savedTime && savedKey === key) {
        ...
        // 如果保存失败，显示警告但不阻止加载
        logger.warn('⚠️ localStorage 保存失败，可能影响冲突检测');
      }
    } catch (e) {
      logger.error('❌ 保存版本信息时发生错误:', e);
      // 即使保存失败，也不阻止加载数据
    }
  } else {
    logger.warn('⚠️ 无法记录版本信息：数据中没有 updated_at 字段');
  }
...
function renderCatList() {
  ...
  panel.querySelectorAll(".cloud-item").forEach((el) => {
    ...
    logger.log(`✅ 云端历史加载成功: 显示最新 1 条快照`);
    
  } catch (err) {
    logger.error('❌ 渲染云端历史失败:', err);
    panel.innerHTML =
...
function renderCatList() {
  ...
  const cats = readCats();
  logger.log('🎨 渲染分类列表，分类数量:', cats.length);
  // ✅ 详细显示要渲染的分类数据
  logger.log('📋 要渲染的分类详情:', cats.map(cat => ({
    id: cat.id,
    name: cat.name,
    color: cat.color
  })));
  if (cats.length === 0) {
    logger.warn('⚠️ 警告：分类列表为空，无法渲染');
  ...
  logger.log('🔨 生成的 HTML 内容（前500字符）:', htmlContent.substring(0, 500));
  logger.log('🔨 生成的 HTML 内容长度:', htmlContent.length);
  logger.log('🔨 分类列表容器元素:', list);
  logger.log('🔨 分类列表容器 innerHTML 长度（渲染前）:', list.innerHTML.length);
  
  list.innerHTML = htmlContent;
  
  // ✅ 调试：检查渲染后的内容
  logger.log('🔨 分类列表容器 innerHTML 长度（渲染后）:', list.innerHTML.length);
  logger.log('🔨 渲染后的分类行数量:', list.querySelectorAll('.cat-row').length);
  logger.log('🔨 渲染后的分类名称元素数量:', list.querySelectorAll('.cat-name').length);
  
  // ✅ 调试：显示每个分类名称元素的内容
  list.querySelectorAll('.cat-name').forEach((el, idx) => {
    logger.log(`🔨 分类 ${idx} 名称元素:`, {
      element: el,
...
function bindEvents() {
  ...
  if (!gridBody) {
    logger.error('❌ gridBody 元素不存在，无法绑定事件');
    return;
  }
  ...
  gridBody.addEventListener("blur", async (e) => {
    ...
    logger.log(`📝 blur 事件: field=${field}, value=${val}`);
  ...
  gridBody.addEventListener("change", async (e) => {
    logger.log("🔔 change 事件触发，目标：", e.target.tagName, e.target);
    
    const sel = e.target.closest('select[data-field="row_color"]');
    logger.log("🔍 找到的 select 元素：", sel);
    
    if (!sel) {
      logger.log("❌ 未找到匹配的 select 元素");
      return;
    }
    
    const id = sel.getAttribute("data-id");
    const newColor = sel.value;
    logger.log(`✅ 分类改变: ID=${id}, 新分类=${newColor}`);
    
    // 更新数据库
    const row = await db.rows.get(id);
    logger.log("📖 读取的行数据：", row);
  ...
  if (clearSearchBtn) {
    ...
    logger.log('✅ clearSearchBtn 事件已绑定');
  } else {
    logger.error('❌ clearSearchBtn 元素不存在');
  }
  ...
  if (searchInput) {
    ...
    logger.log('✅ searchInput 事件已绑定');
  } else {
    logger.error('❌ searchInput 元素不存在');
  }
  ...
  if (btnAdd) {
    ...
    logger.log('✅ btnAdd 事件已绑定');
  } else {
    logger.error('❌ btnAdd 元素不存在');
  }

  // ✅ 导入 CSV 按钮
  const btnImportCSV = $("#btnImportCSV");
  const csvFile = $("#csvFile");
  if (btnImportCSV && csvFile) {
    btnImportCSV.addEventListener("click", () => {
      setActiveFunction("import");
      csvFile.click();
    });
    logger.log('✅ btnImportCSV 事件已绑定');
  } else {
    logger.error('❌ btnImportCSV 或 csvFile 元素不存在');
  }
  ...
  if (btnExportCSV) {
    ...
    logger.log('✅ btnExportCSV 事件已绑定');
  } else {
    logger.error('❌ btnExportCSV 元素不存在');
  }
  ...
  if (btnSaveCloud) {
    btnSaveCloud.addEventListener("click", async () => {
      try {
        logger.log('🖱️ 点击了"保存云端"按钮');
        logger.log('✅ 确认：已移除 prompt() 弹窗，将直接使用自动生成的快照名称');
        setActiveFunction("saveCloud");
        const original = btnSaveCloud.textContent;
        btnSaveCloud.disabled = true; 
        btnSaveCloud.textContent = '⏳ 保存中...';
      
      // ✅ 针对安卓设备：确保获取最新代码（强制清除可能的缓存）
      // 注意：这里只是日志，实际代码中已经没有 prompt() 了
      logger.log('🔍 检查代码版本：当前 app.js 应该不包含 prompt() 调用');
      ...
      btnSaveCloud.textContent = '✅ 已保存';
      setTimeout(() => { btnSaveCloud.textContent = original; btnSaveCloud.disabled = false; }, 800);
    } catch (error) {
      logger.error('❌ 按钮点击事件处理失败:', error);
      alert("保存失败：按钮事件处理错误\n\n错误信息：" + (error.message || String(error)) + "\n\n请查看浏览器控制台（F12）获取详细信息");
      if (btnSaveCloud) { btnSaveCloud.disabled = false; }
    }
    });
    logger.log('✅ btnSaveCloud 事件已绑定');
  } else {
    logger.error('❌ btnSaveCloud 元素不存在');
  }

  // ✅ 加载云端按钮
  const btnLoadCloud = $("#btnLoadCloud");
  if (btnLoadCloud) {
    btnLoadCloud.addEventListener("click", async () => {
      setActiveFunction("loadCloud");
      const original = btnLoadCloud.textContent;
      const panel = $("#cloudHistoryPanel");
      try {
        btnLoadCloud.disabled = true; 
        btnLoadCloud.textContent = '⏳ 加载中...';
      ...
      if (btnLoadCloud) {
        ...
        btnLoadCloud.textContent = original;
      }
    }
    });
    logger.log('✅ btnLoadCloud 事件已绑定');
  } else {
    logger.error('❌ btnLoadCloud 元素不存在');
  }
  ...
  if (btnDeleteData && deleteDataMenu) {
    ...
    if (menuClearAll) {
      menuClearAll.addEventListener("click", async (e) => {
        ...
        const original = btnDeleteData ? btnDeleteData.textContent : '';
        try {
          if (btnDeleteData) { btnDeleteData.disabled = true; btnDeleteData.textContent = '⏳ 清空中...'; }
          await db.rows.clear();
          await refreshFilters();
          await renderTable();
          if (btnDeleteData) btnDeleteData.textContent = '✅ 已清空';
        } finally {
          if (btnDeleteData) {
            setTimeout(() => { btnDeleteData.disabled = false; btnDeleteData.textContent = original; }, 800);
          }
        }
      });
      logger.log('✅ menuClearAll 事件已绑定');
    } else {
      logger.error('❌ menuClearAll 元素不存在');
    }
    
    // 删除空白数据
    const menuDeleteEmpty = $("#menuDeleteEmpty");
    if (menuDeleteEmpty) {
      menuDeleteEmpty.addEventListener("click", async (e) => {
        ...
        const original = btnDeleteData ? btnDeleteData.textContent : '';
        try {
          if (btnDeleteData) { btnDeleteData.disabled = true; btnDeleteData.textContent = '⏳ 删除中...'; }
          const ids = emptyRows.map(r => r.id);
          await db.rows.bulkDelete(ids);
          await refreshFilters();
          await renderTable();
          if (btnDeleteData) btnDeleteData.textContent = '✅ 已删除';
        } finally {
          if (btnDeleteData) {
            setTimeout(() => { btnDeleteData.disabled = false; btnDeleteData.textContent = original; }, 800);
          }
      ...
      logger.log('✅ menuDeleteEmpty 事件已绑定');
    } else {
      logger.error('❌ menuDeleteEmpty 元素不存在');
    }
  } else {
    logger.error('❌ btnDeleteData 或 deleteDataMenu 元素不存在');
  }
  ...
  if (btnCategories) {
    ...
    logger.log('✅ btnCategories 事件已绑定');
  } else {
    logger.error('❌ btnCategories 元素不存在');
  }
  ...
  if (btnCatAdd && catName && catColor) {
    ...
    logger.log('✅ btnCatAdd 事件已绑定');
  } else {
    logger.error('❌ btnCatAdd 或相关元素不存在');
  }
  ...
  if (btnView) {
    ...
    logger.log('✅ btnView 事件已绑定');
  } else {
    logger.error('❌ btnView 元素不存在');
  }
  ...
  const checkInterval = setInterval(() => {
    ...
    if (checkCount >= 15 || hasUserName) {
      ...
      if (hasUserName) {
        logger.log('✅ 用户名显示检查完成，已显示用户名');
      } else {
        logger.warn('⚠️ 用户名显示检查完成，但未找到用户名');
      }
    }
  }, 500);
    ...
    btnLogout.addEventListener("click", async () => {
      ...
      if (supabase) {
        try {
          ...
          logger.log('✅ Supabase session 已清除');
          
          // ✅ 等待一下，再次检查并清除
          await new Promise(resolve => setTimeout(resolve, 200));
          const { data: { session } } = await supabase.auth.getSession();
          if (session) {
            logger.log('⚠️ Session 仍然存在，再次清除');
            await supabase.auth.signOut();
          }
        } catch (err) {
          logger.error('退出登录时出错:', err);
        }
      }
  ...
  if (btnImportExport && importExportMenu) {
    ...
    if (menuImport) {
      menuImport.addEventListener("click", async (e) => {
        e.stopPropagation();
        importExportMenu.style.display = 'none';
        const original = btnImportExport ? btnImportExport.textContent : '';
        try {
          if (btnImportExport) { 
            btnImportExport.disabled = true; 
            btnImportExport.textContent = '⏳ 导入中...'; 
          }
          if (csvFile) csvFile.click();
        } finally {
          if (btnImportExport) {
            setTimeout(() => { 
              btnImportExport.disabled = false; 
              btnImportExport.textContent = original; 
            }, 800);
          }
        }
      });
      logger.log('✅ menuImport 事件已绑定');
    } else {
      logger.error('❌ menuImport 元素不存在');
    }
    
    // ✅ 菜单导出按钮
    const menuExport = $("#menuExport");
    if (menuExport && btnExportCSV) {
      menuExport.addEventListener("click", async (e) => {
        e.stopPropagation();
        importExportMenu.style.display = 'none';
        btnExportCSV.click();
      });
      logger.log('✅ menuExport 事件已绑定');
    } else {
      logger.error('❌ menuExport 或 btnExportCSV 元素不存在');
    }
  } else {
    logger.error('❌ btnImportExport 或 importExportMenu 元素不存在');
  }
  ...
  if (btnSnapshotBrowser) {
    ...
    logger.log('✅ btnSnapshotBrowser 事件已绑定');
  } else {
    logger.error('❌ btnSnapshotBrowser 元素不存在');
  }
  ...
  if (menuClearAll) {
    menuClearAll.addEventListener("click", async (e) => {
      e.stopPropagation();
      if (deleteDataMenu) deleteDataMenu.style.display = 'none';
      clearActiveFunction();
      if (!confirm("确定清空本地所有数据？此操作不可恢复。")) {
        return;
      }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    await dbReadyPromise;
    if (!db.isOpen()) {
      logger.log('⏳ 等待 IndexedDB 数据库打开...');
      await db.open();
      logger.log('✅ IndexedDB 数据库已打开');
    } else {
      logger.log('✅ IndexedDB 数据库已就绪');
    }
  } catch (err) {
    logger.error('❌ IndexedDB 数据库打开失败:', err);
    // 继续执行，避免阻塞应用
  }
  ...
  if (!window.currentUser) {
    ...
    if (storedName && storedId) {
      ...
      logger.log('✅ DOMContentLoaded: 从 localStorage 恢复用户信息', window.currentUser);
    }
  }
  ...
  if (!window.isAdmin && localStorage.getItem('xhs_is_admin') === 'true') {
    window.isAdmin = true;
    logger.log('✅ 从 localStorage 恢复管理员权限');
  }
  ...
  if (currentUserNameEl) {
    ...
    if (userName && userName !== '匿名用户') {
      ...
      currentUserNameEl.style.visibility = 'visible';
      currentUserNameEl.style.opacity = '1';
      logger.log('✅ DOMContentLoaded: 立即更新用户名显示', userName);
    }
  }
  ...
  if (sortSelect) {
    ...
    state.sortBy = "owner";
    sortSelect.value = "owner";
    logger.log('✅ 初始化排序设置: 按所属人排序 (owner)');
  } else {
    // 如果下拉框不存在（可能在手机端被隐藏），也确保 state.sortBy 是 "owner"
    state.sortBy = "owner";
    logger.log('✅ 初始化排序设置: 按所属人排序 (owner) - 下拉框不存在');
  }
  ...
  // ✅ 在渲染之前再次确认排序设置
  await refreshFilters();
  logger.log('🔍 渲染前最终确认排序方式:', state.sortBy);
  await renderTable();
  ...
  setTimeout(() => {
    ...
    if (isAdmin) {
      if (btnApproval) {
        ...
        logger.log('✅ 延迟检查：显示审核管理按钮');
      }
      if (btnUser) {
        btnUser.style.display = 'inline-block';
        logger.log('✅ 延迟检查：显示用户管理按钮');
      }
    }
            ...
            setTimeout(async () => {
              try {
                ...
                if (typeof window.checkForUpdate === 'function' && window.serviceWorkerRegistration) {
                  logger.log('✅ 调用全局更新检查函数（自动更新）');
                  await window.checkForUpdate(window.serviceWorkerRegistration);
                } else {
                  logger.warn('⚠️ 更新检查函数不可用，可能需要等待页面完全加载');
                  // ✅ 如果函数不可用，延迟重试
                  setTimeout(async () => {
                    if (typeof window.checkForUpdate === 'function' && window.serviceWorkerRegistration) {
                      logger.log('✅ 延迟后调用更新检查函数（自动更新）');
                      await window.checkForUpdate(window.serviceWorkerRegistration);
                    }
                  }, 2000);
                }
              } catch (err) {
                logger.error('❌ 自动触发更新检查失败:', err);
                // 如果自动更新失败，显示更新提示让用户手动更新
                if (typeof window.showUpdateNotification === 'function') {
                  logger.log('⚠️ 自动更新失败，显示更新提示');
...
function startStatusBarVersionCheck() {
  ...
  statusBarCheckInterval = setInterval(() => {
    logger.log('⏰ 定期检查版本更新（每2分钟）');
    updateStatusInfoBar();
  }, 2 * 60 * 1000); // 2分钟 = 120000毫秒
  
  logger.log('✅ 已启动状态栏版本检查定时器（每2分钟）');
}
...
async function checkAndUpdateSnapshot() {
  if (!supabase) {
    logger.log('⚠️ Supabase 未配置，跳过快照版本检查');
    return;
  }
  
  // ✅ 关键修复：防止重复调用（苹果手机可能快速连续触发）
  if (isCheckingSnapshot || snapshotCheckInProgress) {
    logger.log('⚠️ 快照检查正在进行中，跳过重复调用');
    return;
  }
  ...
  try {
    ...
    if (!localSnapshotTime) {
      logger.log('ℹ️ 没有本地快照记录，检查是否需要从云端加载');
      ...
      if (hasRefusedLoad && refusedTime) {
        ...
        if (hoursSinceRefused < 24) {
          logger.log(`ℹ️ 用户已在 ${Math.floor(hoursSinceRefused)} 小时前拒绝加载云端快照，跳过提示`);
          return;
        } else {
          // 超过24小时，清除拒绝标志，允许再次提示
          logger.log('ℹ️ 拒绝标志已过期（超过24小时），清除标志');
          localStorage.removeItem('snapshot_load_refused');
          localStorage.removeItem('snapshot_load_refused_time');
        }
      }
    ...
    if (cloudSnapshotTime > localSnapshotTimeNum) {
      ...
      if (existingOverlay) {
        logger.warn('⚠️ 检测到残留的快照更新确认对话框遮罩层，强制移除以恢复显示');
        existingOverlay.remove();
      }
      // ✅✅✅ [修复结束]
      ...
      // ✅ 自动加载最新快照
      logger.log('✅ 开始自动加载最新快照...');
      await cloudLoad(localSnapshotKey);
      
      // ✅ 显示提示
      alert(`✅ 已自动加载最新快照\n\n保存人：${savedBy}\n更新时间：${cloudTimeStr}`);
    } else {
      logger.log('✅ 本地快照已是最新版本');
    }
  } catch (err) {
    logger.error('❌ 检查快照版本失败:', err);
  } finally {
    // ✅ 关键修复：清除检查标志，允许下次检查
    // ✅ 延迟清除，避免快速连续调用
    setTimeout(() => {
...
function startSnapshotVersionCheck() {
  ...
  snapshotCheckInterval = setInterval(() => {
    logger.log('⏰ 定期检查快照版本（每2分钟）');
    checkAndUpdateSnapshot();
  }, 2 * 60 * 1000); // 2分钟 = 120000毫秒
  
  logger.log('✅ 已启动快照版本检查定时器（每2分钟）');
}
...
/* =========================
 * 15. 实时软件版本监听 (Realtime Version Check) - [新增]
 * ========================= */

async function initRealtimeVersionCheck() {
  if (!supabase) return;

  logger.log('📡 [App] 启动实时版本监听 (Realtime)...');
  ...
  const channel = supabase
    ...
    .on(
      'postgres_changes',
      {
        ...
        schema: 'public',
        table: 'app_settings',
        filter: 'key=eq.min_version'
      },
      (payload) => {
        const newVersion = payload.new.value;
        const currentVersion = window.APP_VERSION;

        logger.log(`📡 [Realtime] 收到版本更新信号: ${newVersion}, 当前: ${currentVersion}`);
...
function startLoadCloudAutoClose() {
  ...
  logger.log('⏰ 启动云端加载自动关闭定时器（1分钟后自动关闭）');
  
  // 1分钟后自动关闭
  loadCloudAutoCloseTimer = setTimeout(() => {
    // 检查是否仍然处于激活状态
    if (state.activeFunction === 'loadCloud') {
      logger.log('⏰ 1分钟超时，自动关闭云端加载激活状态');
